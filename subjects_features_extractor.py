...
from cleanfeatures.funcs import processing_drugs
...
parser.add_argument('-table', '--list', help='delimited list input table names to extract subject data',
                    type=str, required=True)
args = parser.parse_args()
different_tables = [item for item in args.list.split(',')]
different_filenames = [item.split('_')[0] for item in different_tables]
...
if __name__ == '__main__':
    for tab_name, file_name in zip(different_tables, different_filenames):
        print("file_name", file_name)
        if tab_name in ['CHARTEVENTS', 'OUTPUTEVENTS', 'LABEVENTS']:
            if tab_name == "CHARTEVENTS":
                print(f"[INFO]: Extracting subjects helpers from Table: {tab_name}, Duration up to 1 hour")
                data_extraction_from_tables_char_lab_out(args.data_path, tab_name,
                                                         file_name.lower(), args.output_path,
                                                         features_lists.CHART_VARIABLES)

            elif tab_name == "OUTPUTEVENTS":
                print(f"[INFO]: Extracting subjects helpers from Table: {tab_name}")
                data_extraction_from_tables_char_lab_out(args.data_path, tab_name,
                                                         file_name.lower(), args.output_path,
                                                         features_lists.CHART_VARIABLES)
            else:
            ...
            if tab_name in ['INPUTEVENTS_CV', 'INPUTEVENTS_MV']:
                print(f"[INFO]: Extracting subjects helpers from Table: {tab_name}")
                data_extraction_inputevents_tables(args.data_path, tab_name,
                                                   file_name.lower(), args.output_path,
                                                   features_lists.drugs_list)

"""
How to run the script to extract helpers from the main tables:
